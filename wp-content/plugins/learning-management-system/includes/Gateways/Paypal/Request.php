...
class Request {
	...
	protected function get_line_item_args_single_item( $order ) {
		...
		$this->add_line_item( $all_items_name ? $all_items_name : __( 'Order', 'learning-management-system' ), 1, $this->number_format( $order->get_total(), $order ), $order->get_order_number() );
	...
	protected function add_line_item( $item_name, $quantity = 1, $amount = 0.0, $item_number = '' ) {
		...
		$item = apply_filters(
			...
			array(
				'item_name'   => html_entity_decode( masteriyo_trim_string( $item_name ? wp_strip_all_tags( $item_name ) : __( 'Item', 'learning-management-system' ), 127 ), ENT_NOQUOTES, 'UTF-8' ),
