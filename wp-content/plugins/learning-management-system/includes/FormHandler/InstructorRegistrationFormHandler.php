...
class InstructorRegistrationFormHandler {
	...
	public function process() {
		try {
			...
			$is_user_registration_form_enabled = ( new Addons() )->is_active( 'user-registration-integration' ) && method_exists( Helper::class, 'is_registration_form_replaceable' ) && Helper::is_registration_form_replaceable( 'instructor' );
	...
	protected function register_instructor() {
		...
		if ( masteriyo_registration_is_generate_password() ) {
			...
			if ( masteriyo_is_email_verification_enabled() ) {
				...
				masteriyo_add_notice( __( 'Your account was created successfully. Your login details have been sent to your email address.', 'learning-management-system' ) );
			}
		}
	...
	protected function validate_form( $data ) {
		...
		if ( ! masteriyo_registration_is_generate_password() ) {
			...
			if ( $data['password'] !== $data['confirm-password'] ) {
				$error->add( 'passwords_do_not_match', __( 'The passwords doesn\'t match.', 'learning-management-system' ) );
			}
		}
