...
function masteriyo_string_to_bool( $string ) {
	...
	if ( ! is_string( $string ) ) {
		return false; // Unable to convert, return false.
	}
...
function masteriyo_price( $price, $args = array() ) {
	...
	if ( masteriyo_string_to_bool( $args['show_price_free_text'] ) && 0 === intval( masteriyo_round( $price ) ) ) {
		 ...
		 */
		$formatted_price = apply_filters( 'masteriyo_price_free_text', _x( 'Free', 'Course price text', 'learning-management-system' ), $price );
	} else {
		if ( $args['html'] ) {
