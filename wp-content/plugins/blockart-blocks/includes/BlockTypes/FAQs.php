...
class FAQs extends AbstractBlock {
	...
	public function render($attributes, $content, $block)
	{
		...
		if ('1' ==  $attributes['enableSchema']) {
			...
			if ($faq_data) {
				...
				$schema_html = '<script type="application/ld+json">' . json_encode($schema_json) . '</script>';
	...
	/**
	 * Extracts questions and anwers from the content and return them as array pairs.
	 *
	 * @param string $content Block Content.
	 * @return array
	 */
	protected function get_faq_data($content = '')
	{
		...
		if (!empty($content)) {
			...
			if (isset($matches[1]) && isset($matches[2])) {
				return array_map(function ($question, $answer) {
